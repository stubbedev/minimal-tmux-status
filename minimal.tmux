...
if [ "$indicator_state" = true ]; then
	indicator_active=$(get_tmux_option "@minimal-tmux-indicator-str-active" "  tmux  ")
else
	indicator_active=""
fi
...
status_left=$(get_tmux_option "@minimal-tmux-status-left" "#[bg=default,fg=default,bold]#{?client_prefix,,${indicator_active}}#[bg=${bg},fg=black,bold]#{?client_prefix,${indicator},}#[bg=default,fg=default,bold]")
